from bi_threshold.CTL.causal_tree.ctl.binary_ctl import *
...
class TriggerNode(CTLearnNode):
    ...
    def __init__(self, trigger_d=0.0,trigger_u=0.0, **kwargs):
        # ----------------------------------------------------------------
        # Causal tree node
        # ----------------------------------------------------------------
        super().__init__(**kwargs)

        self.trigger_d = trigger_d
        self.trigger_u = trigger_u
...
class TriggerTree(CTLearn):
    ...
    def _eval(self, train_y, train_t, val_y, val_t):
        ...
        if self.old_trigger_code:
            ...
            train_effect, best_trigger_d, best_trigger_u = tau_squared_trigger(train_y, train_t, self.min_size, self.quartile)
            if train_effect <= -np.inf:
                return return_val

            #val_effect = ace_trigger(val_y, val_t, best_trigger)
            val_effect = ace_trigger(val_y, val_t, best_trigger_d,best_trigger_u)
            ...
            #return best_obj, best_trigger, best_mse
            return best_obj, best_trigger_d,best_trigger_u, best_mse
        
    def get_triggers(self, x):
        def _get_features(node: TriggerNode, observation):
            if node.is_leaf:
                return node.trigger_d,node.trigger_u
            else:
        ...
        num_test = x.shape[0]
        triggers_d = np.zeros(num_test)
        triggers_u = np.zeros(num_test)

        for i in range(num_test):
            test_example = x[i, :]
            triggers_d[i],triggers_u[i] = _get_features(self.root, test_example)

        return triggers_d,triggers_u
